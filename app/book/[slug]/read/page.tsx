...
export default function BookReadPage() {
  ...
  const pdfUrl = book?.pdfPath ? `/api/books/${encodeURIComponent(slug)}/pdf` : null;
  ...
  useEffect(() => {
    ...
    async function loadPdf() {
      ...
      try {
        ...
        if (!cancelled) setError(e instanceof Error ? e.message : "Ошибка загрузки PDF");
      } finally {
  ...
  const renderPage = useCallback(
    async (pageNum: number) => {
      const pdf = pdfDocRef.current as { getPage: (n: number) => Promise<{ getViewport: (o: { scale: number }) => { width: number; height: number }; render: (o: { canvasContext: CanvasRenderingContext2D; viewport: unknown }) => { promise: Promise<void> } }> } | null;
    ...
    <div className="mx-auto flex min-h-[80vh] max-w-4xl flex-col px-4 py-6">
      ...
      {error && pdfUrl && (
        <div className="flex flex-1 flex-col gap-2">
          <p className="text-sm text-amber-700">Встроенный просмотр недоступен. Откройте PDF в новой вкладке или смотрите ниже:</p>
          <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="text-gold hover:underline text-sm">
            Открыть PDF в новой вкладке
          </a>
          <iframe src={pdfUrl} title={book.title} className="flex-1 min-h-[600px] w-full rounded-xl border border-gray-200" />
        </div>
      )}
